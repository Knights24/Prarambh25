    ...
    const handleScroll = () => {
      ...
      for (let i = sections.length - 1; i >= 0; i--) {
        ...
        if (section && section.offsetTop <= scrollPosition) {
          ...
          // Show logo only after reaching the events section
          if (
            sections[i] === "events" ||
            sections[i] === "registration" ||
            sections[i] === "about" ||
            sections[i] === "contact"
          ) {
            setShowLogo(true)
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        >
          <Link href="#home">
            <img src="/prarambh-logo.png" alt="Prarambh Logo" className="h-10 w-auto" />
          </Link>
        </motion.div>
        ...
        <nav className="hidden md:flex space-x-8">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "relative px-2 py-1 text-sm font-medium transition-all duration-300 font-audiowide",
        ...
        <div className="container mx-auto px-4 py-4 flex flex-col space-y-4">
          {navItems.map((item) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "py-2 text-left transition-colors font-audiowide",
                activeSection === item.href.substring(1)
                  ? "text-blue-400 text-glow-blue"
